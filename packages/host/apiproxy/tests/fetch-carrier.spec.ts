  ...
  return {
    sessions: {
      ...
      async retitleAuto(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { accepted: false } } }
      },
      async fork(request) {
...
describe('unary round trip (handler ⇄ client, no network)', () => {
  ...
  it('round-trips the browse listing and creation calls through the wire form', async () => {
    ...
    expect(listed.result).toEqual({
      ok: true,
      value: { path: '/w', home: '/w', crumbs: [{ name: '/', path: '/', hidden: false, kind: 'directory' }], entries: [], truncated: false },
    })
