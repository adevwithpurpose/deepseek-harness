...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const crumbsOf = (path: string): { name: string; path: string; hidden: boolean; kind: 'directory' | 'file' }[] => {
  ...
  const api: ApiProxy = {
    sessions: {
      ...
      retitleAuto: (request) => {
        const missing = requireSession(request)
        if (missing !== undefined) return missing
        // The offline fixture mounts no titler: report no provider revision.
      ...
      listDirectory: (request) => {
        ...
        return ok(request, {
          ...
          entries: [...children].sort((a, b) => a.localeCompare(b))
            .map(name => ({ name, path: target === '/' ? `/${name}` : `${target}/${name}`, hidden: name.startsWith('.'), kind: 'directory' as const })),
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'session.retitleAuto': return this.api.sessions.retitleAuto(request)
