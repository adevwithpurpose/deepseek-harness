...
| `@deepseek-ai/dsh-tool-delegate` | `delegate` | `ctx.rolePolicy`, `ctx.subagents` | `child session lifecycle` | - | Deployment role policy resolves provider, model chain, preset, persona, tool policy, and depth; these execution knobs are absent from the model schema. |
...
Delegate an objective to a configured specialist role. Role policy owns preset, provider, model, tools, persona, and recursion limits.
...
```json
{
  ...
  "properties": {
    "role": {
      "type": "string",
      "description": "Configured specialist role id."
    },
    "objective": {
      "type": "string",
      "description": "Concrete objective for the specialist."
    },
    "acceptance_criteria": {
      "type": "array",
      "description": "Observable completion checks.",
      "items": {
        "type": "string"
      }
    },
    "scope": {
      "type": "array",
      "description": "Paths or subjects the specialist owns.",
      "items": {
        "type": "string"
      }
    },
    "background": {
      "type": "boolean",
      "description": "Return a continuable child id immediately. Defaults to true."
    }
  },
  "required": [
    "role",
...
Source: [`packages/role/tool-delegate/src/index.ts`](../packages/role/tool-delegate/src/index.ts)

Deployment role policy resolves provider, model chain, preset, persona, tool policy, and depth; these execution knobs are absent from the model schema.
